...
open class FormatBar: UIView {
    ...
    fileprivate let hideKeyboardBgView = UIView()
    public var hideKeyboardItem: UIButton? = nil {
        didSet {
            guard let item = hideKeyboardItem else {
                return
            }
            hideKeyboardBgView.removeFromSuperview()
            item.addTarget(self, action: #selector(handleToggleHideKeyboardAction(_:)), for: .touchUpInside)
            addSubview(hideKeyboardBgView)
            hideKeyboardBgView.addSubview(item)
        }
    }
    open var hideKeyboardItemHandler: (() -> Void)? = nil
    ...
    public init() {
        ...
        self.layer.shadowColor = UIColor(hexString: "D5D8DE")?.cgColor
...
//        addSubview(bottomDivider)
    ...
    open override func layoutSubviews() {
        ...
        guard let item = self.hideKeyboardItem else {
            return
        }
        hideKeyboardBgView.backgroundColor = .white
        let bgHW = self.bounds.height
        hideKeyboardBgView.frame = CGRect(x: self.frame.width - bgHW, y: 0, width: bgHW, height: bgHW)
        hideKeyboardBgView.layer.shadowOffset = CGSize(width: 0, height: 0)
        hideKeyboardBgView.layer.shadowColor = UIColor(hexString: "DBDCE3")?.cgColor
        hideKeyboardBgView.layer.shadowOpacity = 0.6
        item.frame = hideKeyboardBgView.bounds
    }
    ...
    @IBAction func handleToggleHideKeyboardAction(_ sender: FormatBarItem) {
        hideKeyboardItemHandler?()
    }
...
private extension FormatBar {
    ...
    func configureConstraints() {
        ...
        @unknown default:
            fatalError("Error: FormatBar configureConstraints(): Unknown layoutDirection")
        }
...
private extension FormatBar.Constants {
    ...
    static let trailingButtonMargin = CGFloat(16)
