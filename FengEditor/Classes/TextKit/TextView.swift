...
//        HeaderFormatter(headerLevel: .h6, placeholderAttributes: [:]),
    ...
    ///
    open var typingAttributesSwifted: [NSAttributedString.Key: Any] {
        get {
            return NSAttributedString.Key.convertFromRaw(convertFromNSAttributedStringKeyDictionary(typingAttributes))
        }
        set {
            typingAttributes = convertToNSAttributedStringKeyDictionary(NSAttributedString.Key.convertToRaw(newValue))
        }
    }
    ...
    private func commonInit() {
        ...
        font = defaultFont
        linkTextAttributesSwifted = [.underlineStyle: NSNumber(value: NSUnderlineStyle.single.rawValue),
                                     .foregroundColor: self.tintColor ?? UIColor.black]
    ...
    private func setupMenuController() {
        let pasteAndMatchTitle = NSLocalizedString("无格式粘贴", comment: "Paste without Formatting Menu Item")
    ...
    public func getHTML(prettify: Bool = true) -> String {
        ...
        return html//storage.getHTML(prettify: prettify)
    }
...
//        HeaderFormatter(headerLevel:.h6),
    ...
    private var linkFormatter = LinkFormatter()
    ...
    open func setLink(_ url: URL, title: String, target: String? = nil, inRange range: NSRange) {
        ...
        let originalColor = getOriginalForegroundColor(from: attributedText, at: range, limitedLength: self.text.count - 1)
        linkFormatter = LinkFormatter(target: target, originalFontColor: originalColor)
        linkFormatter.attributeValue = url

        let attributes = linkFormatter.apply(to: typingAttributesSwifted)
    ...
    open func setLink(_ url: URL, target: String? = nil, inRange range: NSRange) {
        ...
        apply(formatter: linkFormatter, atRange: range, remove: false)
    }

    /// Helper function to get the original foreground color
    fileprivate func getOriginalForegroundColor(from _attributedString: NSAttributedString, at range: NSRange, limitedLength: Int) -> UIColor? {
        if _attributedString.length == 0 { return nil }
        return _attributedString.attributes(at: min(range.location, limitedLength > 0 ? limitedLength : 0), effectiveRange: nil)[NSAttributedString.Key.foregroundColor] as? UIColor
    }
    ...
    open func removeLink(inRange range: NSRange) {
        apply(formatter: linkFormatter, atRange: range, remove: true)
    }


    // MARK: - Embeds

    public func replace(at range: NSRange, with attachment: NSTextAttachment) {
    ...
    @discardableResult
    open func replaceWithEmotion(_ emotion: UIImage, at range: NSRange, emotionKey: String) -> EmotionAttachment {
        let attactment = EmotionAttachment()
        attactment.image = emotion
        attactment.emotionKey = emotionKey
        attactment.bounds = CGRect(x: 0, y: 0, width: emotionWH, height: emotionWH)
        replace(at: range, with: attactment)
        return attactment
    }
    private let emotionWH: CGFloat = 30
...
private extension TextView {
    ...
    private func removeParagraphAttributes(at range: NSRange) {
        ...
        for formatter in formatters {
            let activeTypingAttributes = NSAttributedString.Key.convertFromRaw(convertFromNSAttributedStringKeyDictionary(super.typingAttributes))
            ...
            super.typingAttributes = convertToNSAttributedStringKeyDictionary(NSAttributedString.Key.convertToRaw(updatedTypingAttributes))
